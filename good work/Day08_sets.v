...
Fixpoint intersection (l1 l2 : natset) : natset :=
  match l1 with
  | [] => []
  | x :: l1' => if member x l2 then set_insert x (intersection l1' l2)
...
Definition set_equal (l1 l2 : natset) : bool :=
  match l1 , l2 with
  | [] , [] => true
  | [], _ => false
  | _, [] => false
  | x :: l1', y :: l2' => subset l1 l2 && subset l2 l1
  end. 
...
Definition empty_natset : natset := empty.

(** What about [set_insert]? *)
Definition set_insert : nat -> natset -> natset := insert.
(* Do not modify the following line: *)
...
Fixpoint union (t1 t2 : natset) : natset :=
  match t1 with
  |  empty   => t2
  | node l v r => ( union r (set_insert v (union l t2)))
  end.
End BSTSets.
